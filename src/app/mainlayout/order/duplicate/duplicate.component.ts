...
fetch()
{
  this.Loader = this.loader.show();
	...
	this.client.fetc_duplicate(json).subscribe((data: any) => {
      ...
      if (obj.status == 200) {
       ...
       // this.add_shipment.get('f_com_name').patchValue(this.PickupAddlist[0].pickup_id);
       //   this.add_shipment.get('from_gst').patchValue(this.PickupAddlist[0].gst_number); 
...
set_row_value()
{
 ...
 existing_prods(prods:any, wt:any)
 {
   ...
   prods.forEach(s => {
    var disc = 0;
    if(s.discount != undefined && s.discount != '')
    {
      disc = s.discount;
    }
    else
...
get_order_Data()
{
  this.client.get_order_data(localStorage.getItem("token")).subscribe((data: any) => {
      ...
      if (obj.status == 200) {
       ...
       //console.log(this.add_shipment.get('f_com_name').value);
      }
      else
      {
        this.tostr.error(obj.message);
...
re_calculate_weight()
{
   ...
   for(let i:number = 0; i < a.length; i++)
   {
     var prod_weight = 0;
     var roe_weight = [];
     var b:any = <FormArray> a.controls[i].get('package_details').value;
         for(let j:number = 0; j < b.length; j++)
         {
           var data = parseFloat(b[j].weight);
           roe_weight.push(data);
         }  
      prod_weight=roe_weight.reduce((a, b) => a + b, 0);
      ...
      {
        prod_weight = prod_weight *  parseFloat(shipment);
        a.controls[i].get('total_weight').setValue(prod_weight,{emitEvent:false});
      }
      else{
        //alert("success")
        a.controls[i].get('total_weight').setValue(prod_weight,{emitEvent:false});
