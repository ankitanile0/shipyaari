  ...
  fetch()
{
  ...
  this.Loader = this.loader.show();
	...
	this.client.edit_shipment(json).subscribe((data: any) => {
      var response = data._body;
      var obj = JSON.parse(response);
      if (obj.status == 200) {
         this.Loader = this.loader.hide();
       this.order_id = obj.result.processOrder.order_id  
       this.temp_del_address = obj.result.processOrder.billing_address;
       this.temp_del_address = JSON.parse(this.temp_del_address);
       //this.PickupAddlist = obj.result.processOrder.shipping_address;
       this.consignmentdata = obj.result.processOrder.product_details;
       this.consignmentdata = JSON.parse(this.consignmentdata);
       this.template_List = JSON.parse(obj.result.template_list)
       //console.log("prod",this.consignmentdata);
       //console.log("template",this.template_List);
   		var roe_total = [];
   		for(let i:number =0; i < this.consignmentdata.length; i++)
   		{
   		 var data1 = parseFloat(this.consignmentdata[i].total);
         roe_total.push(data1);
        }  
       this.total_price=roe_total.reduce((a, b) => a + b, 0);
       console.log(this.total_price);
         for(let k:number = 0; k < this.consignmentdata.length; k++)
         {
           if(k > 0)
           {
              this.addChildTable(0); 
           } 
         }
         
       this.set_data(this.consignmentdata);
...
set_data(data)
{
  ...
  pickup_id:"",//new
  to_pincode:this.temp_del_address.postcode,
  to_landmark:"",
  to_address:this.temp_del_address.address_1,
  to_address2:this.temp_del_address.address_2,
  to_state:this.temp_del_address.state,//new
  to_city:this.temp_del_address.city,//new
  to_country:this.temp_del_address.country,//new
  to_company:this.temp_del_address.company,//new
  customer_lname:this.temp_del_address.last_name,//new
  customer_name:this.temp_del_address.first_name,
  customer_email:this.temp_del_address.email,
  customer_contact_no:this.temp_del_address.phone,
  ship_date:new Date(),
  package_type:"",
  package_content:"",
  ...
  no_of_packages:1,
  total_price_set:this.total_price,
  channel:"",
  ...
  cour_part_id:"",//new
  serv_serv_id:"",//new
  product_data:this.ExistingPackage(this.consignmentdata)
  })
  ...
  // console.log(a);
    var formArray = [];
       formArray.push({
          ...
          package_height:'',
          total_weight:'',
          package_details:this.existing_prods(a),
        });
  return formArray;
 }

  existing_prods(prods:any)
 {
...
set_row_value()
{
  ...
  get_order_Data()
  {
    this.client.get_order_data(localStorage.getItem("token")).subscribe((data: any) => {
      ...
      if (obj.status == 200) {
        ...
        //this.add_shipment.get('to_company').patchValue(this.deliveryAddress[0])
  ...
  get_del_address_by_mobile()
  {
    ...
    this.client.del_add_details_mobile(json).subscribe((data: any) => {
      ...
      if (obj.status == 200) {
        ...
        // this.add_shipment.get('customer_contact_no').setValue(obj.result.mobile_number);
        //this.add_shipment.get('customer_lname').setValue(name[1]);
        this.add_shipment.get('customer_name').setValue(obj.result.firstname);
        this.add_shipment.get('customer_email').setValue(obj.result.email);
        this.add_shipment.get('to_landmark').setValue(obj.result.landmark);
        this.add_shipment.get('to_address').setValue(obj.result.address1);
        this.add_shipment.get('to_address2').setValue(obj.result.address2);
        this.add_shipment.get('to_pincode').setValue(obj.result.postcode);
        this.add_shipment.get('to_city').setValue(obj.result.city);
        this.add_shipment.get('to_state').setValue(obj.result.state);
        this.add_shipment.get('to_country').setValue(obj.result.country);
        var country = obj.result.country;
        if(country == 1)
        {
          this.add_shipment.get('to_country').setValue("India");
  ...
  get_template_list()
  {
    this.client.templatelist(localStorage.getItem("token")).subscribe((data: any) => {
      ...
      if (obj.status == 200) {
        ...
        //this.template_List = this.template_List.reverse()
        //this.tempid = this.template_List[0].id;
      }
      else
      {
        //this.tostr.error(obj.message);
  ...
  fetch_template(i:number,j:number, aa)
  {
    ...
    { 
      ...
      this.bsetPrice = false;
      this.aval_serv_table = null; 
      var prod = this.add_shipment.get('product_data') as FormArray;
      console.log(prod.length);
      for(let i:number = 0; i < prod.length; i++)
      {
        prod.controls[i].get('total').patchValue(null);
      }
    }
   else
   {
  ...
  addMainTable(){
    const count:any = (< HTMLInputElement > document.getElementById('count')).value;
   // alert(count);
    if(count != 1 && count != '')
    {
...
add_button:boolean = false
  ...
  Chek_Identical()
  {
    ...
    {
      this.addMainTable();
      this.add_shipment.get('product_data').setValue('');
    }
    else
    {
      this.remTable();
  ...
  fetch_rate(data, serve, part){
    ...
    var identical = this.add_shipment.get('identical').value; if(identical == true){ identical =1; }else{ identical =2;}
    var insurance = this.add_shipment.get('Insurance').value; if(insurance == true){ insurance = 'yes'; }else{ insurance = 'no';}
    ...
    this.client.fetch_rate_order(json).subscribe((data: any) => {
      ...
      if (obj.status == 200) {
        ...
        //console.log(this.bsetPrice);
        if(this.indicator == 1)
        {
          this.aval_serv_table = [];
          this.aval_serv_table.push(obj.result.result[0]);
         // console.log(this.aval_serv_table);
        }
  ...
  toggleShow() { 
    this.add_shipment.get('serv_serv_id').setValue('');
    this.add_shipment.get('cour_part_id').setValue('');
    this.add_shipment.get('Serv_id').setValue('');
    this.add_shipment.get('part_id').setValue('');
    this.new_fetch_service="Courier_services"
    this.aval_serv_table = '';
    this.service_flag = false;
    this.rad_serv = null;
    this.service_flag = false;
  ...
  service_radio()
  {
    ...
    this.add_shipment.get('Serv_id').setValue('');
    this.add_shipment.get('part_id').setValue('');
    this.new_fetch_service="Services"
    this.aval_serv_table = '',
    this.service_flag = false;
    this.service_flag = false;
    this.rad_serv = null;
  ...
  radio_courier()
  {
    ...
    this.add_shipment.get('Serv_id').setValue('');
    this.add_shipment.get('part_id').setValue('');
    this.new_fetch_service="Courier"
    this.aval_serv_table = ''
    this.service_flag = false;
    this.service_flag = false;
    this.rad_serv = null;
  ...
  { 
    ...
    this.add_shipment.get('Serv_id').setValue('');
    this.add_shipment.get('part_id').setValue('');
    this.new_fetch_service="Best_Service"
    this.indicator = 1;
    this.service_flag = false;
    this.service_flag = false;
    this.rad_part = null;
  ...
  change_cheap()
  {
    ...
    this.add_shipment.get('Serv_id').setValue('');
    this.add_shipment.get('part_id').setValue('');
    this.new_fetch_service="Cheapest"
    this.indicator = 1;
    this.service_flag = false;
    this.service_flag = false;
    this.rad_part = null;
  ...
  auto_set_billable_weight()
  {
   ...
   // console.log("fetch", cour_name, this.fetch_service)
  ...
  Get_product_data()
  {
    ...
    a.forEach(s => {
      formArray.push({
        ...
        total:  s.total,
        package_details:this.Get_package_details(count,s.package_details)  
      });
      count++;
    })
    return formArray;
  ...
  Get_package_details(count:number, b:any)
  {
    var formArray = [];
    b.forEach(s => {
      formArray.push({
  ...
  Submit()
  {
    ...
    var paymode = a.COD;
    a.customer_name
    this.cust_name = a.customer_name
      ...
      var json = 
    {
      ...
      service_type: this.new_fetch_service,
      ...
      delivery_address: {
        to_customer_name: this.cust_name,
        to_customer_gstin: "",
