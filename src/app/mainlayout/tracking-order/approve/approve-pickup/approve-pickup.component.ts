...
@Component({
  selector: 'app-approve-pickup',
  templateUrl: './approve-pickup.component.html',
  styleUrls: ['./approve-pickup.component.css']
})
export class ApprovePickupComponent implements OnInit {
  Loader:any;
  file:any;
  modal:BsModalRef;
  sheet:any = null;
...
get_data()
{
  ...
  this.client.approve_pickup(localStorage.getItem('token')).subscribe((data: any) => {
...
search_track()
{
  ...
  this.client.search_pickup_tracking(json).subscribe((data: any) => {
      var response = data._body;
      var obj = JSON.parse(response);
      if (obj.status == 200) {
       //this.file = obj.result;
        this.Loader = this.loader.hide();
        this.list_data = obj.result.tracking_number;
        //var temp = this.list;
...
check:any[] = [];
  ...
  download_repor()
  {
    ...
    this.client.approve_pickup_download(json).subscribe((data: any) => {
      var response = data._body;
      var obj = JSON.parse(response);
      if (obj.status == 200) {
        this.tostr.success(obj.message);
...
creat_batch()
{
  ...
  this.client.app_pickup_batch(json).subscribe((data: any) => {
      var response = data._body;
      var obj = JSON.parse(response);
      if (obj.status == 200) {
        this.tostr.success(obj.message);
